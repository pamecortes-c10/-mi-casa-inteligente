...
NEWS_API_KEY     = os.environ.get('NEWS_API_KEY', '8109282e84634017822fec3dc078c191')  # NewsAPI key
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # ── GET /news → proxy NewsAPI (evita exponer la key al cliente) ──
        if path == '/news':
            country  = qs.get('country',  ['mx'])[0]
            pageSize = qs.get('pageSize', ['6'])[0]
            key = qs.get('key', [NEWS_API_KEY])[0]
            if not key:
                self.send_json({'error': 'Falta NEWS_API_KEY en el servidor'}, 400)
                return
            try:
                nurl = (f'https://newsapi.org/v2/top-headlines'
                        f'?country={country}&pageSize={pageSize}&apiKey={key}')
                req = urllib.request.Request(nurl, headers={'User-Agent': 'MiCasaInteligente/1.0'})
                with urllib.request.urlopen(req) as r:
                    data = json.loads(r.read())
                self.send_json(data)
            except urllib.error.HTTPError as e:
                self.send_json({'error': f'NewsAPI HTTP {e.code}: {e.reason}'}, e.code)
            except Exception as e:
                self.send_json({'error': str(e)}, 500)
            return
